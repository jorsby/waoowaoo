...
用户 → Frontend (WorkspaceAssistantPanel)
  ...
  runtime.ts: createProjectAgentChatResponse()
    ...
    └── streamText(model, prompt, tools, maxSteps=12)
              │
              ...
              │
              └── LLM 观察 tool result → 决定下一步 (最多 12 步)
...
| 序号 | 工作项                                                               | 投入 | 收益    |
| ---- | -------------------------------------------------------------------- | ---- | ------- |
| 1    | **Tool 错误不 throw，返回结构化错误**（解锁自愈闭环）                | 低   | 🔥 极高 |
...
| 7    | **分级确认：medium risk 无需 confirmed gate**                        | 低   | 中      |
...
| API 对齐报告           | `docs/operation_tool_api_alignment_report.md`                           |
